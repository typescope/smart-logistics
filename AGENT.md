...
4. Choose a source for each product. A product may be sourced from several
   ...
   Say which source you chose and why whenever it is not the preferred one.
5. Group proposed lines by supplier and call `saveDraftOrder` once per supplier.
   Quantities must satisfy the case size and minimum order quantity of the
   supplier you are ordering from.
6. Explain the arithmetic, assumptions, saved draft IDs, and products that need
...
Runtime validation covers physical facts only: whether the supplier sources the
product, that supplier's case size and minimum order quantity, storage capacity,
duplicate lines, and products already in an open draft. Nothing machine-checks the rules, so honoring them is your
